# -*- coding: utf-8 -*-
"""
Created on Mon Oct 26 21:34:56 2020
...
def main(X_train, X_test, y_train, y_test):
    ...
    for i, test_item in enumerate(X_test):
        ...
        for j in range(len(knearest)):
            lines.append(np.stack((test_item, X_train[knearest[j]])))

        #y_predict[i] = 0          # this just classifies everything as 0
        predict = []
        for j in range(len(knearest)):
            predict.append(y_train[knearest[j]])
        y_predict[i] = np.round(np.mean(predict))
